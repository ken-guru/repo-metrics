...
function parseArgs() {
  ...
  node dist/repo_test_growth.js <repo-url-or-path> [--branch main] [--first-parent] [--include-merges]
    ...
    for (let i = 1; i < a.length; i++) {
        const t = a[i];
        if (t === "--branch")
            args.branch = a[++i] ?? null;
        else if (t === "--first-parent")
            args.firstParent = true;
        else if (t === "--include-merges")
            args.includeMerges = true;
        else if (t === "--sample-every")
            args.sampleEvery = parseInt(a[++i], 10);
        else if (t === "--max-commits")
            args.maxCommits = parseInt(a[++i], 10);
        else if (t === "--max-file-bytes")
            args.maxFileBytes = parseInt(a[++i], 10);
        else if (t === "--msg-avg-window")
            args.msgAvgWindow = parseInt(a[++i], 10);
        else if (t === "--verbose")
            args.verbose = true;
        else if (t === "--plotly-src")
            args.plotlySrc = a[++i] ?? null;
        else if (t === "--csv-decimals")
            args.csvDecimals = parseInt(a[++i], 10);
        else if (t === "--dry-run")
            args.dryRun = true;
        else if (t === "--keep-temp")
            args.keepTemp = true;
            ...
            args.outputPrefix = a[++i];
        else {
            console.error(`Unknown arg: ${t}`);
            process.exit(2);
        }
    }
    if (!Number.isFinite(args.sampleEvery) || args.sampleEvery < 1)
        args.sampleEvery = 1;
    if (!Number.isFinite(args.msgAvgWindow) || args.msgAvgWindow < 1)
        args.msgAvgWindow = 50;
    if (!Number.isFinite(args.csvDecimals) || args.csvDecimals < 0)
        args.csvDecimals = 2;
...
export function sanitizeRepoForDisplay(s) {
    ...
    try {
        return path.basename(path.resolve(s));
    }
    catch {
        return s;
    }
 ...
 */
export function copyPlotlyToAssets(assetsDir, htmlPath, options) {
    ...
    try {
        ...
        if (verbose)
            console.error(`Copied Plotly from ${candidate} to ${destPath}`);
...
export function sanitizeOutputPrefix(p) {
...
export function pathShouldSkip(p) {
...
function isTestPath(p) {
    ...
    if (TEST_FILE_PREFIXES.some(pref => base.startsWith(pref)))
        return true;
    if (TEST_FILE_SUFFIXES.some(sfx => base.endsWith(sfx)))
        return true;
    return false;
}
...
export function seemsBinary(buf) {
    return buf.includes(0); // NUL byte heuristic
}
export function countCodeLoc(text, ext) {
    let count = 0;
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (!s)
            continue;
        if (prefixes.some(pref => s.startsWith(pref)))
            continue;
        count++;
    }
    return count;
}
export function countDocLoc(text) {
    let count = 0;
    for (const line of text.split(/\r?\n/)) {
...
export function countTestCases(text, ext) {
...
async function ensureRepo(repoArg, opts) {
...
async function getRev(git, branchArg) {
    if (branchArg)
        return branchArg;
    try {
...
async function listCommits(git, rev, opts) {
    const args = ["rev-list", "--reverse"];
    if (!opts.includeMerges)
        args.push("--no-merges");
    if (opts.firstParent)
        args.push("--first-parent");
    if (opts.maxCommits && Number.isFinite(opts.maxCommits))
        args.push(`--max-count=${opts.maxCommits}`);
    args.push(rev);
    const out = await git.raw(args);
    return out.split("\n").map(s => s.trim()).filter(Boolean);
}
async function listTree(git, commit) {
    ...
    for (const line of out.split("\n")) {
        ...
        const sha = meta[2];
        let size = null;
        if (meta.length >= 4 && /^\d+$/.test(meta[3]))
            size = parseInt(meta[3], 10);
        entries.push({ mode, type, sha, size, path: filePath });
    }
    return entries;
}
async function catBlob(git, repoPath, sha, maxBytes) {
...
export async function computeMetricsForCommit(git, repoPath, commit, blobCache, maxFileBytes) {
    ...
    for (const e of entries) {
        ...
        if (!isDoc && pathShouldSkip(p))
            continue;
        if (e.size !== null && e.size > maxFileBytes)
            continue;
...
/** ---------- Plot (Plotly HTML) ---------- */
function writePlotlyHtml(outPath, repoLabel, rev, rows, msgAvgWindow, plotlySrc) {
    ...
    const resolvedPlotlySrc = (function () {
        if (plotlySrc)
            return plotlySrc;
        if (typeof globalThis.__repo_plotly_src === 'string')
            return globalThis.__repo_plotly_src;
        try {
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < commits.length; i++) {
            ...
            const { nonTestLoc, totalTests, docLoc } = await computeMetricsForCommit(git, repoPath, c, blobCache, args.maxFileBytes);
        ...
        if (typeof args.assetsDir !== 'undefined') {
            ...
            try {
                ...
                if (verbose)
                    ...
                    process.stderr.write('Failed to copy assets; using CDN.\n');
            }
        }
