...
import { writeCsv, copyPlotlyToAssets } from '../src/repo-metrics';
...
describe('output module (unit)', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'test-out-'));
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'test-asset-'));
